...
export function WorkerRow({ row, weekDates, todayStr, isAr, compact, onCellClick }: WorkerRowProps) {
  ...
  return (
    <div
      className={cn(
        "grid border-b border-border hover:bg-muted/60 transition-colors",
        compact
    ...
    >
      ...
      <div
        className={cn(
          "sticky start-0 z-10 bg-card border-e border-border flex items-center gap-2.5 px-3 py-2.5",
        )}
      >
        <Avatar className={cn(compact ? "h-8 w-8" : "h-9 w-9", "ring-2 ring-card shadow-sm")}>
          {worker.photo_url && <AvatarImage src={worker.photo_url} alt={worker.full_name} />}
          <AvatarFallback className="bg-present/10 text-present text-xs font-bold">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-xs text-muted-foreground truncate">{worker.job_title}</p>
        </div>
      </div>

      {/* Day cells */}
      {weekDates.map((date) => (
        <div key={date} className="p-1 border-e border-muted/40">
          <ShiftBlock
      ...
      <div
        className={cn(
          ...
          "bg-present/10 border-s border-present/30"
        )}
      >
        <div className="flex items-center gap-1">
          <Clock className={cn("text-present shrink-0", compact ? "w-3 h-3" : "w-3.5 h-3.5")} />
          <span className={cn("font-bold text-present tabular-nums", compact ? "text-sm" : "text-base")}>
            {weekTotalHours.toFixed(1)}
          </span>
        </div>
        <span className="text-xs font-medium text-present/80 mt-0.5">
          {isAr ? "ساعة" : "heures"}
        </span>

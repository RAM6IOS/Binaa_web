...
export default function RFQWizardPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8 py-8">
      <div>
        <Link href="/marketplace" className="text-primary hover:underline flex items-center gap-2 mb-4 text-sm font-medium">
      ...
      <div className="flex items-center justify-between mb-8 relative">
        ...
        <div className="absolute top-1/2 left-0 h-1 bg-primary -z-10 -translate-y-1/2 transition-all" style={{ width: `${(step - 1) * 50}%` }} />
        
        {[1, 2, 3].map((s) => (
          <div key={s} className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm outline outline-4 outline-background ${s <= step ? 'bg-primary text-primary-foreground' : 'bg-border text-muted-foreground'}`}>
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          {step === 1 && (
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <input type="text" className="w-full border p-2 rounded focus:ring-2 focus:ring-primary" placeholder={isAr ? 'مثال: أعمال بناء أساسية في ولاية الجزائر...' : 'Ex: Gros œuvres à Alger...'} />
              </div>
              <div className="space-y-2">
                <label className="text-sm font-medium">{isAr ? 'نوع الخدمة' : 'Type de service'}</label>
                <select className="w-full border p-2 rounded focus:ring-2 focus:ring-primary">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                 <div className="space-y-2">
                  ...
                  <input type="number" className="w-full border p-2 rounded focus:ring-2 focus:ring-primary" />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-medium">{isAr ? 'موعد التسليم' : 'Date limite'}</label>
                  <input type="date" className="w-full border p-2 rounded focus:ring-2 focus:ring-primary" />
                </div>
              </div>
            </div>
          )}
          ...
          {step === 2 && (
            <div className="space-y-4 text-muted-foreground text-center py-10 border-2 border-dashed rounded-lg">
              <p>[File Upload & Detailed Description Mockup]</p>
            </div>
          )}

          {step === 3 && (
            <div className="space-y-4 text-muted-foreground text-center py-10 bg-primary/5 rounded-lg">

...
export default function MarketplacePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div className="space-y-8">
      {/* Hero Section */}
      <div className="bg-primary/5 dark:bg-primary/10 border rounded-lg p-8 md:p-12 flex flex-col items-center text-center space-y-6">
        ...
        <div className="w-full max-w-3xl flex flex-col sm:flex-row gap-2 mt-4">
          <div className="relative flex-1">
            ...
            <input 
              ...
              className="w-full pl-10 pr-4 py-3 rounded-lg border bg-background text-base rtl:pr-10 rtl:pl-4 focus:ring-2 focus:ring-primary focus:outline-none"
            />
          </div>
          <div className="relative w-full sm:w-48 hidden sm:block">
            <MapPin className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground rtl:right-3 rtl:left-auto" />
            <input 
              type="text" 
          ...
          <div>
            ...
            <div className="space-y-4">
              <div className="space-y-2 text-sm">
                <p className="font-medium text-foreground">{isAr ? 'نوع الخدمة' : 'Type de service'}</p>
                {['البناء والطرق', 'التصميم المعماري', 'توريد المواد'].map(f => (
                  <label key={f} className="flex items-center gap-2 text-muted-foreground">
                    <input type="checkbox" className="rounded" /> {f}
                  </label>
                ))}
              </div>
              <div className="space-y-2 text-sm border-t pt-4">
                <p className="font-medium text-foreground">{isAr ? 'حجم الشركة' : 'Taille de l\'entreprise'}</p>
        ...
        <div className="flex-1 space-y-4">
          ...
          <div className="grid lg:grid-cols-2 gap-4">
            {CONTRACTORS.map(contractor => (
              <Card key={contractor.id} className="hover:shadow-sm transition-shadow duration-300 flex flex-col">
                <CardContent className="pt-6 flex-1">
                  <div className="flex justify-between items-start">
                    <div className="flex gap-4">
                      <div className="w-12 h-12 rounded-full border bg-muted flex items-center justify-center font-bold text-muted-foreground shrink-0">
                        {contractor.name[0]}
                      </div>
                      <div>
                        <Link href={`/marketplace/contractor/${contractor.id}`} className="font-semibold text-lg leading-tight hover:text-primary flex items-center gap-2">
                          {contractor.name}
                          {contractor.verified && <ShieldCheck className="w-4 h-4 text-success" />}
                        </Link>
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    {contractor.services.map(s => (
                      <span key={s} className="px-2 py-1 bg-muted text-muted-foreground rounded text-xs">
                        {s}
                      </span>
                    ))}
                  </div>
                </CardContent>
                <CardFooter className="border-t bg-muted p-4 shrink-0 flex gap-2">

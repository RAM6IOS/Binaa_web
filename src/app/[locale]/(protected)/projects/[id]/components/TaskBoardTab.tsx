    ...
    <div ref={setNodeRef} style={style} className={`bg-white dark:bg-slate-950 p-4 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 transition-all hover:border-blue-400 group ${isDragging ? 'z-50 shadow-2xl scale-105' : ''}`}>
      <div className="flex justify-between items-start mb-3">
        <div className="flex items-start gap-3 flex-1 min-w-0">
          <div {...attributes} {...listeners} className="mt-1 cursor-grab active:cursor-grabbing text-slate-300 hover:text-blue-500 transition-colors shrink-0">
        ...
        <AddTaskDialog isAr={isAr} projectId={projectId} onSuccess={onRefresh} task={task} trigger={
          <button className="p-2.5 md:p-1.5 text-slate-400 hover:text-blue-600 bg-slate-50 dark:bg-slate-800 rounded-lg active:scale-95 transition-all shrink-0" aria-label="Edit task"><Edit2 className="w-4 h-4 md:w-3 md:h-3" /></button>
        } />
      </div>
      <div className="space-y-3 pl-7">
        ...
        <div className="flex justify-between items-center text-[10px] text-slate-500 border-t pt-2 mt-2">
          <span className="flex items-center gap-1.5 truncate"><User className="w-3 h-3 text-blue-500 shrink-0" />{assignedWorker?.full_name || (isAr ? 'غير معين' : 'Non assigné')}</span>
...
function MobileTaskCard({ task, isAr, workers, projectId, onRefresh }: { task: ProjectTask, isAr: boolean, workers: Worker[], projectId: string, onRefresh: () => void }) {
  ...
  return (
    <div className="bg-white dark:bg-slate-950 p-4 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 active:bg-slate-50 dark:active:bg-slate-900 transition-colors">
      <div className="flex justify-between items-start mb-2">
        <div className="flex-1 min-w-0 mr-3">
          <h4 className="text-sm font-bold leading-tight truncate">{task.title}</h4>
        </div>
        <AddTaskDialog isAr={isAr} projectId={projectId} onSuccess={onRefresh} task={task} trigger={
          <button className="p-2.5 text-slate-400 hover:text-blue-600 bg-slate-50 dark:bg-slate-800 rounded-lg active:scale-95 transition-all shrink-0" aria-label="Edit task"><Edit2 className="w-4 h-4" /></button>
        } />
      </div>
    ...
    <Card className="animate-in fade-in duration-300 md:duration-500 border-none shadow-none bg-transparent overflow-hidden">
      <CardHeader className="px-0 flex flex-col md:flex-row md:items-center justify-between gap-3 md:gap-4 pb-6 md:pb-8">
        <div>
          <CardTitle className="text-xl md:text-2xl font-black text-slate-900 flex items-center gap-3">
            <div className="p-2 bg-blue-600 rounded-xl"><CheckCircle2 className="w-5 h-5 md:w-6 md:h-6 text-white" /></div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="hidden md:flex bg-white border dark:bg-slate-900 p-1 rounded-xl shadow-sm">
      ...
      <CardContent className="px-0">
        ...
        <div className="hidden md:block">
          {view === 'kanban' ? (
            <div className="flex gap-5 min-h-[600px] overflow-x-auto pb-6 custom-scrollbar">
        ...
        <div className="md:hidden space-y-4">
          ...
          <div className="flex items-center gap-2 overflow-x-auto no-scrollbar pb-1">
            <button
              ...
              className={`shrink-0 px-3.5 py-2 rounded-xl text-xs font-bold transition-all ${mobileFilter === 'all' ? 'bg-blue-600 text-white shadow-md' : 'bg-white dark:bg-slate-900 text-slate-600 border border-slate-200 dark:border-slate-700'}`}
            >
            ...
            {columns.map(col => (
              <button
                ...
                className={`shrink-0 px-3.5 py-2 rounded-xl text-xs font-bold transition-all ${mobileFilter === col.id ? 'bg-blue-600 text-white shadow-md' : 'bg-white dark:bg-slate-900 text-slate-600 border border-slate-200 dark:border-slate-700'}`}
              >
          ...
          <div className="space-y-3">
            {filteredTasks.length === 0 ? (
              <div className="py-16 text-center border-2 border-dashed border-slate-200 rounded-2xl">
                ...
                <p className="text-sm font-bold text-slate-400">{isAr ? 'لا توجد مهام في هذا التصنيف' : 'Aucune tâche dans cette catégorie'}</p>
              </div>
            ) : (

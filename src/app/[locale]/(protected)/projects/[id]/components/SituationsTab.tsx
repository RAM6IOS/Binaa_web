  ...
  return (
    <div className="space-y-6" dir={isAr ? "rtl" : "ltr"}>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 bg-card rounded-lg border text-center p-6">
          <div className="p-4 bg-primary/10 text-primary rounded-lg mb-3">
          ...
          <div className="hidden md:block bg-card rounded-lg border shadow-sm overflow-hidden">
            <table className="w-full text-right border-collapse text-xs">
              ...
              <tbody className="divide-y">
                {situations.map((sit) => (
                  <tr key={sit.id} className="hover:bg-muted/60 transition-colors">
                    <td className="p-4 font-bold text-sm text-foreground">
                      N° {sit.situation_number}
                    </td>
                    <td className="p-4 font-bold capitalize text-foreground">
                      {sit.situation_type}
                    </td>
                    ...
                    <td className="p-4 font-bold text-success text-sm">
                      {(sit.net_a_payer || 0).toLocaleString()} <span className="text-xs">DZD</span>
                    </td>
                    <td className="p-4">
                      <span className={`px-2.5 py-1 rounded-full text-xs font-bold inline-flex items-center gap-1 ${
                        sit.status === "validated" ? "bg-success/10 text-success" : "bg-warning/10 text-warning"
                      }`}>
                    ...
                    <td className="p-4 text-center">
                      <div className="flex items-center justify-center gap-2">
                        <Button
                          ...
                          className="h-8 px-3 rounded-lg gap-1 text-xs font-bold text-primary hover:bg-primary/10"
                        >
            ...
            {situations.map((sit) => (
              ...
              >
                <div className="flex justify-between items-start">
                  <div>
                    <span className="text-xs font-bold text-primary">Situation N° {sit.situation_number}</span>
                ...
                <div className="text-xs text-muted-foreground space-y-1 pt-2 border-t">
                  <p><strong className="text-foreground">{isAr ? "تاريخ الإيقاف:" : "Arrêtée au:"}</strong> {sit.arretee_au}</p>
                  <p><strong className="text-foreground">{isAr ? "النوع:" : "Type:"}</strong> {sit.situation_type}</p>
                </div>
                ...
                <div className="flex items-center justify-end gap-2 pt-2 border-t">
                  <Button
                    ...
                    size="sm"
                    onClick={() => setSelectedSituationId(sit.id)}
                    className="h-8 rounded-lg text-xs font-bold gap-1 text-primary"
                  >
